...
describe("Game security & recovery e2e", () => {
  ...
  beforeAll(async () => {
    const [hashA, hashB] = await Promise.all([
      bcrypt.hash(userA.password, 10),
      bcrypt.hash(userB.password, 10),
    ]);
  ...
  it("returns seat-redacted state per user", async () => {
    const [resA, resB] = await Promise.all([
      request(app).get(`/api/v1/games/${gameId}`).set("Authorization", `Bearer ${tokenA}`),
      request(app).get(`/api/v1/games/${gameId}`).set("Authorization", `Bearer ${tokenB}`),
    ]);
